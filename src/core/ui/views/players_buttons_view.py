...
class PlayersButtonsView(discord.ui.View):
    ...
    def add_player_button(self, player: Player):
        ...
        async def button_callback(interaction: discord.Interaction):
            ...
            print(self.cog.first_captain, self.cog.second_captain)
            if not self.cog.first_captain or not self.cog.second_captain:
                await interaction.response.send_message(
                    "Nenhum capitão foi escolhido ainda.", ephemeral=True
                )
                return

            if interaction.user.id == self.cog.first_captain.discord_id:
                self.cog.first_captain_team.append(player)
            elif interaction.user.id == self.cog.second_captain.discord_id:
                self.cog.second_captain_team.append(player)
            else:
                await interaction.response.send_message(
            ...
            await interaction.response.edit_message(
                embed=choose_captains_embed(
                    self.cog.first_captain_team,
                    self.cog.second_captain_team,
                ),
                view=self,
            )
